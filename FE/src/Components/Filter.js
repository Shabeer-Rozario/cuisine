...
class Filter extends React.Component {
  ...
  pageNavigation = (res) => {
    this.props.history.push(`/details?restaurent=${res._id}`)
  }
        ...
        <div className="container">
          <div>
            {restaurents.length > 0 ?
              <div className="row heading d-flex justify-content-left">
          ...
          return  <div className="resultPanel" onClick={() => this.pageNavigation(item)}>
          ...
          <div className="row lowerSection">
            <div className="col-8">
              <div className="resultsAddress">CUISINES: {item.cuisine.map(cuisineItem => `${cuisineItem.name}`)}</div>
              <div className="resultsAddress">COST FOR TWO: &#8377; {item.min_price}</div>
            </div>
          </div>
        </div>
        }) : <div className="noresults">No Results Here...</div>}
